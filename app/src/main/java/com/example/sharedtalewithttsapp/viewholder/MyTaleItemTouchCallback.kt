...
// 리사이클러뷰를 파라미터로 받았다.
class MyTaleItemTouchCallback(private val recyclerView: RecyclerView) : ItemTouchHelper.SimpleCallback(
    ...
    override fun onMove(
        ...
        (recyclerView.adapter as MyTaleAdapter).moveItem(viewHolder.layoutPosition, target.layoutPosition)
        return true
    }
    // 아이템을 스와이프 할떄
    override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
        Log.d(TAG, "ItemTouchCallback - onSwiped() called");
        (recyclerView.adapter as MyTaleAdapter).removeItem(viewHolder.layoutPosition)
    }
